...
Render free instances spin down after ~15 minutes of inactivity, causing cold starts (~30–60s). A pinger is included to keep the RAG and backend services warm:

#### Option A: GitHub Actions Pinger (recommended, free)
A scheduled workflow lives at `.github/workflows/pinger.yml`. It pings both service `/health` endpoints every 10 minutes.

1. Open the GitHub repo → **Settings → Secrets and variables → Actions**.
2. Add repository variables or secrets:
   - `RAG_HEALTH_URL` — e.g. `https://shikkhaai-rag.onrender.com/health`
   - `BACKEND_HEALTH_URL` — e.g. `https://shikkhaai-backend.onrender.com/health`
3. The workflow runs automatically via cron; trigger it manually from **Actions** the first time.

> The script uses lightweight `curl` calls with a 30-second timeout. Failures are logged but do not fail the workflow.

#### Option B: Local / Self-hosted Pinger
Run the included Python script anywhere that can reach your services:
...
- `RAG_HEALTH_URL` — defaults to `http://127.0.0.1:8100/health`
- `BACKEND_HEALTH_URL` — defaults to `http://127.0.0.1:8000/health`
- `PING_INTERVAL_SECONDS` — defaults to `600`
- `PING_TIMEOUT_SECONDS` — defaults to `30`

> Even if the pinger misses, the backend falls back to direct Gemini generation using `GEMINI_API_KEY`, and ultimately to mock mode if configured.
